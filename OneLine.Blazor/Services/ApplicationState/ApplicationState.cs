...
namespace OneLine.Blazor.Services
{
    public class ApplicationState : IApplicationState
    {
        ...
        public ApplicationState(NavigationManager navigationManager)
        {
        ...
        public async ValueTask<TUser> GetApplicationUserSecure<TUser>()
        {
            try
            {
                ...
                if (Helpers.Device.IsXamarinPlatform && applicationSession.Equals(ApplicationSession.LocalStorage))
                {
                    SUser = await SecureStorage.GetAsync("SUser");
                    return JsonConvert.DeserializeObject<TUser>(SUser);
                }
                else if (Helpers.Device.IsXamarinPlatform && applicationSession.Equals(ApplicationSession.SessionStorage))
                {
